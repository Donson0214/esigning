  ...
  <div class="sign-page sign-document">
    <header class="builder-topbar">
      ...
      <div class="right-actions">
        ...
        <button class="btn btn-primary" type="button" :disabled="!canSend" @click="signDocument">
          {{ signButtonLabel }}
        </button>
      </div>
    </header>
        ...
        <div class="panel-section">
          ...
          <div class="palette">
            <button
              ...
              :key="field.type"
              :class="['palette-item', activePaletteType === field.type && 'active']"
              type="button"
          ...
          <p class="helper">Click a field type to jump to the next field and fill it.</p>
        </div>
      </aside>
    </section>
    ...
    <div v-if="fieldModalOpen" class="signature-modal">
      ...
      <div class="signature-dialog" role="dialog" aria-modal="true" aria-label="Field input">
        ...
        <div class="signature-area field-area">
          <template v-if="fieldModalField?.type === 'CHECKBOX'">
            <label class="toggle">
              <input v-model="fieldModalChecked" type="checkbox" />
              Checked
            </label>
          </template>
          <template v-else-if="fieldModalField && isChoiceField(fieldModalField) && fieldModalChoices.length">
            <select v-model="fieldModalValue" class="type-input">
              <option v-for="choice in fieldModalChoices" :key="choice" :value="choice">{{ choice }}</option>
            </select>
          </template>
          <template v-else>
            <input v-model.trim="fieldModalValue" :type="fieldModalInputType" class="type-input" />
          </template>
        </div>

        <div class="signature-actions">
          <button class="btn btn-outline" type="button" @click="closeFieldModal">Cancel</button>
          <button class="btn btn-primary" type="button" @click="saveFieldModal">Apply</button>
        </div>
      </div>
    </div>
  </div>
...
const fieldModalField = ref<SigningSessionView['fields'][number] | null>(null);
const fieldModalValue = ref('');
const fieldModalChecked = ref(false);
...
const handlePaletteClick = (type: SigningSessionView['fields'][number]['type']) => {
  ...
  const target = getNextFieldByType(type);
  if (!target) {
  ...
  if (isSignatureField(target)) {
    if (!activeSignature.value) {
      pendingSignatureFieldId.value = target.id;
      void openSignatureModal();
      return;
    }
    placeSignature(target);
    return;
  }
  openFieldModal(target);
};
...
const isChoiceField = (field: SigningSessionView['fields'][number]) =>
  field.type === 'DROPDOWN' || field.type === 'RADIO';

const fieldModalChoices = computed(() =>
  fieldModalField.value ? getFieldChoices(fieldModalField.value) : [],
);

const fieldModalInputType = computed(() => {
  const type = fieldModalField.value?.type;
...
const isFinalStatus = (status?: string) =>
  status === 'SIGNED' || status === 'COMPLETED' || status === 'DECLINED' || status === 'EXPIRED';

const canSend = computed(() => {
  if (!doc.value || !sessionView.value) return false;
  if (isFinalStatus(doc.value.status)) return false;
  return missingSignatureCount.value === 0;
});

const signButtonLabel = computed(() => {
  if (isFinalStatus(doc.value?.status)) return 'Sent';
  return canSend.value ? 'Send' : 'Sign document';
});
...
const getAttachmentMeta = (field: SigningSessionView['fields'][number]) => {
  if (!field.options || typeof field.options !== 'object') return null;
  const attachment = (field.options as Record<string, unknown>).attachment;
  if (!attachment || typeof attachment !== 'object') return null;
  return attachment as { name?: string };
};

const getAttachmentLabel = (field: SigningSessionView['fields'][number]) => {
...
const isFieldFilled = (field: SigningSessionView['fields'][number]) => {
  if (isSignatureField(field)) {
    return Boolean(placements.value[field.id]);
  }
  if (field.type === 'CHECKBOX') {
    return isCheckedValue(field.value);
  }
...
const focusField = (field: SigningSessionView['fields'][number]) => {
  activeFieldId.value = field.id;
  const pageEl = pageRefs.get(field.page);
  const size = pageSizes.value[field.page];
  if (!pageEl || !viewerRef.value) return;
  if (!size) {
    scrollToPage(field.page);
    return;
  }
  const normalized = resolveNormalizedRect(field, size);
  const fieldTop = normalized.y * size.height * scale.value;
  const targetTop = pageEl.offsetTop + fieldTop - 24;
  viewerRef.value.scrollTo({ top: Math.max(0, targetTop), behavior: 'smooth' });
};

const getNextFieldByType = (type: SigningSessionView['fields'][number]['type']) => {
  const matches = fields.value.filter((field) => field.type === type);
  if (matches.length === 0) return null;
  return matches.find((field) => !isFieldFilled(field)) ?? matches[0];
};
...
const handleFieldClick = (field: SigningSessionView['fields'][number]) => {
  ...
  if (isSignatureField(field)) {
    if (!activeSignature.value) {
      pendingSignatureFieldId.value = field.id;
      void openSignatureModal();
      return;
    }
...
const updateFieldValue = (fieldId: string, value: string) => {
  fields.value = fields.value.map((field) =>
    field.id === fieldId ? { ...field, value: value.trim() || null } : field,
  );
};
...
const openFieldModal = (field: SigningSessionView['fields'][number]) => {
  ...
  if (field.type === 'CHECKBOX') {
    fieldModalChecked.value = isCheckedValue(field.value);
    fieldModalValue.value = '';
    return;
  }
  const fallbackValue = field.value ?? resolveFieldValue(field);
  const choices = getFieldChoices(field);
  if (isChoiceField(field) && choices.length > 0) {
    fieldModalValue.value = choices.includes(fallbackValue) ? fallbackValue : choices[0];
    return;
  }
  fieldModalValue.value = fallbackValue;
};
...
const saveFieldModal = () => {
  const field = fieldModalField.value;
  if (!field) return;
  if (field.type === 'CHECKBOX') {
    updateFieldValue(field.id, fieldModalChecked.value ? 'checked' : '');
    closeFieldModal();
    return;
  }
  updateFieldValue(field.id, fieldModalValue.value);
  closeFieldModal();
};
...
const signDocument = async () => {
  if (!doc.value || !sessionView.value || !canSend.value) return;
  errorMessage.value = '';
  ...
  try {
    ...
    if (token.value) {
      try {
        ...
        pdf = await getDocument({ data: response.data, verbosity: 0 }).promise;
      } catch {
        pdf = null;
      }
    }
    ...
    if (!pdf) {
      ...
      try {
        ...
        pdf = await getDocument({ url: doc.value.fileUrl, verbosity: 0 }).promise;
      }
    }
    if (tokenId !== pdfLoadToken.value) return;
...
watch(activeSignature, (next) => {
  if (!next) return;
  if (pendingSignatureFieldId.value) {
    const field = fields.value.find((item) => item.id === pendingSignatureFieldId.value);
    if (field) {
      placeSignature(field, next);
    }
    pendingSignatureFieldId.value = null;
  }
  if (placementCount.value === 0) return;
  const fieldMap = new Map(fields.value.map((field) => [field.id, field]));
